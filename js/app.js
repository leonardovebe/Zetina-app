...
async function loadInventarioVendido() {
  ...
  const { data, error } = await db
    .from('inventario_vendedoras')
    .select('id, prenda_id, fecha_entrega, prendas(numero, nombre, marca, emoji, talla_real, precio_max, gradiente, fotos_prendas(id, url, orden))')
    .eq('vendedora_id', VENDEDORA_ID)
    .eq('estado', 'vendido')
    .order('created_at', { ascending: false });
    ...
    container.innerHTML = `
      <div class="catalog-header">
        <div class="catalog-header-row">
          ...
          <button class="btn-refresh" id="btnRefreshCatalogo" aria-label="Actualizar catálogo">${REFRESH_SVG}</button>
        </div>
    ...
    <div class="catalog-header">
      <div class="catalog-header-row">
        ...
        <div class="header-actions">
          <button class="btn-refresh" id="btnRefreshCatalogo" aria-label="Actualizar catálogo"${modoClientaActivo ? ' hidden' : ''}>${REFRESH_SVG}</button>
    ...
    container.innerHTML = `
      <div class="pedidos-header">
        <div class="section-header-row">
          ...
          <button class="btn-refresh" id="btnRefreshPedidos" aria-label="Actualizar pedidos">${REFRESH_SVG}</button>
        </div>
      </div>
      <div class="catalog-empty">
        <p class="catalog-empty-icon">📦</p>
  ...
  container.innerHTML = `
    <div class="clientes-header">
      <div class="section-header-row">
        ...
        <button class="btn-refresh" id="btnRefreshClientes" aria-label="Actualizar clientes">${REFRESH_SVG}</button>
      </div>
    </div>
...
function getVentaDePrend(prendaId) {
  for (const c of clientes) {
    const comp = (c.compras || []).find(v => v.prendaId === prendaId);
    if (comp) return { ...comp, clienteNombre: c.nombre, clienteId: c.id };
  }
  return null;
}

function buildInvCardVendida(p) {
  const venta    = getVentaDePrend(p.id);
  const fotos    = p.fotos || [];
  const primera  = fotos.find(f => f.url);
  return `
    <article class="inv-card inv-card--vendida" data-id="${p.id}" data-inv-id="${p.invId}">
      <div class="inv-card-img" style="background:${p.gradiente}"${fotos.length ? ` data-galeria-id="${p.id}"` : ''}>
        ${primera ? `<img class="inv-card-foto" src="${primera.url}" alt="${p.nombre}" loading="lazy">` : `<span class="inv-card-emoji" aria-hidden="true">${p.emoji}</span>`}
        <span class="inv-vendida-badge">Vendida</span>
      </div>
      <div class="inv-card-body">
        <p class="inv-card-nombre">${p.nombre}</p>
        <span class="inv-talla-chip">Real&nbsp;<strong>${p.tallaReal || '—'}</strong></span>
        ${venta ? `<p class="inv-vendida-info">${formatPeso(venta.monto)} · ${venta.clienteNombre}</p>` : ''}
        <div class="inv-btn-stack" style="margin-top:0.5rem">
          ${venta ? `<button class="inv-btn inv-btn--desc" data-detalle-vendida="${p.id}">Ver detalle</button>` : ''}
...
function openDetalleVendidaSheet(prendaId) {
  const p     = inventarioVendido.find(x => x.id === prendaId);
  const venta = getVentaDePrend(prendaId);
  if (!p || !venta) { showToast('Sin datos de venta registrados.'); return; }
  const overlay = document.getElementById('detalleVendidaOverlay') || (() => {
    const el = document.createElement('div');
    el.id = 'detalleVendidaOverlay';
    ...
    el.innerHTML = `
      <div class="order-detail-sheet">
        ...
        <div class="sheet-body" id="detalleVendidaBody" style="padding:0 1.25rem 2rem"></div>
      </div>`;
    document.body.appendChild(el);
    el.addEventListener('click', e => { if (e.target === el) el.classList.remove('open'); });
    el.querySelector('.btn-sheet-close').addEventListener('click', () => el.classList.remove('open'));
    return el;
  })();
  document.getElementById('detalleVendidaBody').innerHTML = `
    <p class="melaquedo-prenda">${p.emoji || '👗'} ${p.nombre}</p>
    <div class="vision-maestria-lista" style="margin-top:1rem">
      <div class="vision-maestria-item"><span class="vision-maestria-label">Clienta</span><span class="vision-maestria-valor">${venta.clienteNombre}</span></div>
      <div class="vision-maestria-item"><span class="vision-maestria-label">Precio de venta</span><span class="vision-maestria-valor">${formatPeso(venta.monto)}</span></div>
      <div class="vision-maestria-item vision-maestria-item--last"><span class="vision-maestria-label">Fecha</span><span class="vision-maestria-valor">${formatFecha(venta.fecha)}</span></div>
    </div>`;
  overlay.classList.add('open');
}
...
function renderMisPrendas() {
  ...
  const gridHTML = (() => {
    if (modoClientaPrendasActivo) return inventario.map(buildInvCardClienteMode).join('') || `<p class="inv-empty">Sin prendas disponibles</p>`;
    const activas = renderGrid(inventario);
    if (!verVendidasActivo) return activas;
    const vendidas = inventarioVendido.length ? inventarioVendido.map(buildInvCardVendida).join('') : '';
  ...
  container.innerHTML = `
    ...
    <div class="inv-grid${modoClientaPrendasActivo ? ' inv-grid--clienta' : ''}" id="invGrid">${gridHTML}</div>`;
  ...
  container.querySelector('#btnVerVendidas').addEventListener('click', async () => {
    ...
    if (verVendidasActivo && !inventarioVendido.length) {
      await loadInventarioVendido();
    }
    renderMisPrendas();
  });
  ...
  container.onclick = (e) => {
    ...
    // Prendas vendidas: "Ver detalle" y "Regresar al stock"
    const detalleBtn = e.target.closest('[data-detalle-vendida]');
    if (detalleBtn) { openDetalleVendidaSheet(detalleBtn.dataset.detalleVendida); return; }

    const regresarBtn = e.target.closest('[data-regresar-id]');
    if (regresarBtn) {
      if (!confirm('¿Regresar esta prenda a tu inventario?')) return;
...
function renderCuenta() {
  ...
  console.log('[stats completos]', JSON.stringify(vs));
  console.log('[logros raw]', vs?.logros_obtenidos);
  console.log('[logros tipo]', typeof vs?.logros_obtenidos);
  console.log('[logros parseados]', parseLogrosArray(vs?.logros_obtenidos));
  const logrosResueltos = getLogros();
  console.log('[logros obtenidos en render]', logrosResueltos.filter(l => l.obtenido).map(l => l.id));
  const logrosHTML = logrosResueltos.map(l => `
